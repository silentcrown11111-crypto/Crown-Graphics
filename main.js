document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.service-card, .pricing-card, .sub-card, .portfolio-item').forEach(el => {
        el.style.opacity = '0';
        observer.observe(el);
    });

    // Pricing Toggle Logic & Geo-Location
    ...
    if (quoteForm) {
        quoteForm.addEventListener('submit', (e) => {
            ...
            if (deliveryMethod === 'whatsapp') {
                ...
                alert(`[SIMULATION] Your WhatsApp security code is: ${currentOtp}`);
    ...
    pricingBuyBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (checkoutTotalAmount) checkoutTotalAmount.textContent = `$${price} USD`;
